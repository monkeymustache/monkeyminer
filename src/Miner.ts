...
class Miner extends EventEmitter {
  ...
  sendToMiner(payload: MonkeyMinerResponse) {
    const monkeyminerMessage = JSON.stringify(payload);
    if (this.online && this.ws.readyState === WebSocket.OPEN) {
      try {
        this.ws.send(monkeyminerMessage);
      } catch (e) {
        this.kill();
      }
    }
  ...
  handleMessage(message: string) {
    ...
    switch (data.type) {
      case "auth": {
        const params = data.params as MonkeyMinerLoginParams;
